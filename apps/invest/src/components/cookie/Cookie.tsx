...
import { createEffect, createSignal, Show, VoidComponent, on } from 'solid-js';
import Back from '../../assets/back.svg';

const Cookie: VoidComponent = () => {
  const [storage, setStorage, { remove, clear, toJSON }] =
    createCookieStorage();
  ...
  createEffect(() => {
    if (!storage['showCookies'] || storage['showCookies'] === 'true') {
      setModal(true);
    }
  });

  const handleCookies = () => {
    setStorage('showCookies', 'false');
    setStorage('productDevCookie', productDevCookie() ? 'YES' : 'NO');
    setModal(false);
  };
